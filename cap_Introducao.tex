...
\begin{figure}[H]
	...
	\centering
	\caption{Esquema do sistema e comunicação entre as suas partes}
	\label{fig:system}
\end{figure}
...
Cada jogador conectado ao servidor pode realizar ações que terão efeito no ambiente. As ações são descritas abaixo. \cite{rcssmanual2003}

\subsubsection{Agarrar}

O único jogador com permissão para agarrar ou pegar a bola é o goleiro. O goleiro pode pegar a bola em qualquer direção, desde que ela esteja dentro da área agarrável, definida como um retângulo com comprimento 2.0 e largura 1.0 na direção $\omega$ em que o goleiro deseja agarrar. 

Se o comando de agarrar falhar, o goleiro fica incapacitado de agarrar a bola durante um número pré-determinados de ciclos do jogo. Se o comando tiver sucesso o goleiro pode usar o comando "mover-se" (Seção \ref{sec:move}) para se mover junto com a bola, não podendo, porém, ultrapassar o limite do número de usos deste comando em da bola.
...
O comando de arrancar faz com que haja uma aceleração do jogador na direção do seu corpo. Um parâmetro "potência" determina o valor da aceleração. É importante notar que o comando de arranque é o jeito padrão de movimentar um jogador.

Cada jogador possui uma certa quantidade de energia e o arranque consome a energia do jogador. Ao começo de cada tempo da partida, a energia do jogador é colocada no máximo. Se o jogador acelera para frente, a energia é reduzida pelo valor de "potência". Se o jogador acelera para trás, o custo é maior e a energia é reduzida pelo valor fixo de $-2\times potência$. 
...
O comando de chute recebe dois parâmetros: a força do chute e a direção do chute. Para realizar o comando, a bola precisa ser "chutável", ou seja, estar a uma certa distância do jogador. 

Caso a bola não esteja diretamente a frente do jogador, a força efetiva será reduzida por um fator dependente da posição relativa da bola.
...
O comando virar recebe o ângulo para qual o jogador irá virar. Se o jogador não se mexer, seu ângulo é igual ao ângulo passado no comando. 

O resultado do comando é influenciado pelo momento de inércia e a velocidade linear do jogador, definido nas configurações do servidor. Isso restringe o ângulo de uma rotação efetiva.

\subsubsection{Virar pescoço}

O jogador pode virar seu pescoço de maneira semi-independente de seu corpo. O ângulo do pescoço é relativo ao ângulo do corpo, então caso um comando virar seja executado, o ângulo absoluto do pescoço também mudará. Os ângulos máximo e mínimo em relação ao corpo são definidos na configuração do servidor.
